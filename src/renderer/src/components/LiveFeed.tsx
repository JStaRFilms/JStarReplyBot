    ...
    return (
        <div className="bg-white/70 dark:bg-slate-800/70 backdrop-blur-xl border border-slate-200/60 dark:border-white/10 rounded-2xl p-1 min-h-[400px] max-h-[600px] overflow-y-auto">
            {events.map((event, index) => (
                <div key={index} className="p-4 hover:bg-slate-50 dark:hover:bg-white/5 transition-colors rounded-xl cursor-default group border-b border-transparent hover:border-slate-100 dark:hover:border-white/5 last:border-0">
                    ...
                    <div className={`ml-13 pl-4 border-l-2 ${event.status === 'failed' ? 'border-rose-200 dark:border-rose-500/30' : 'border-indigo-200 dark:border-indigo-500/30'} space-y-1 py-1`}>
